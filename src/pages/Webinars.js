...
function Webinars() {
        ...
        setWebinars((prevWebinars) =>
          prevWebinars.map((webinar) =>
            webinar.webinarId === webinarId
              ? { ...webinar, slots: webinar.slots - 1 }
              : webinar
          )
        );
      })
      .catch((error) => console.error("Error registering for webinar:", error));
  };
  ...
  const handleCancelRegistration = (webinarId) => {
    fetch(
      ...
      .catch((error) => console.error("Error canceling registration:", error));
  };
  ...
  return (
    <div style={containerStyle}>
      <h1 style={headerStyle}>Upcoming Webinars</h1>
      <div style={listStyle}>
        {webinars.map((webinar) => (
          ...
          >
            ...
            {registeredWebinars.includes(webinar.webinarId) ? (
              <button
                style={cancelButtonStyle}
                onClick={() => handleCancelRegistration(webinar.webinarId)}
              >
                Cancel Registration
              </button>
            ) : (
              <button
                ...
                onClick={() => handleRegister(webinar.webinarId)}
              >
                Register
              </button>
            )}
